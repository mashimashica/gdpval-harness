...
class LocalJudgeExecutorTests(unittest.TestCase):
    ...
    def test_codex_judge_is_local_read_only_and_ephemeral(self) -> None:
        with tempfile.TemporaryDirectory() as root:
            command = CodexJudgeExecutor().build_command(self.request(root))
    ...
    def test_claude_judge_restricts_tools_network_and_writes(self) -> None:
        with tempfile.TemporaryDirectory() as root:
            ...
            self.assertEqual(command[command.index("--tools") + 1], "Bash,Read")
            ...
            self.assertEqual(sandbox["filesystem"]["denyWrite"], [str(request.workspace.resolve())])
            self.assertNotIn("--cloud", command)
