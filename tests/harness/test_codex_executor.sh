...
case "${1-}" in
  ...
  exec)
    ...
    if [[ -n "${FAKE_STALE_JUDGE_PATH:-}" && -e "$FAKE_STALE_JUDGE_PATH" ]]; then
      echo "stale judge artifact was still present when replacement execution began" >&2
      exit 96
    fi
...
: >"$args_log"
out="$work/run"
stale="$out/deliverables/task_task-one/repeat_0/finish_params.json"
